    ...
    update(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const userExists = yield database_1.default.query('SELECT * FROM sesion WHERE usuario = ?', [usuario]);
                if (userExists.length === 0) {
                    res.status(404).json({ message: 'Sesión no encontrada' });
                    return;
                }
                // Actualizar la sesión
                const result = yield database_1.default.query('UPDATE sesion SET id_rol = ?, contraseña = ? WHERE usuario = ?', [id_rol, contraseña, usuario]);
